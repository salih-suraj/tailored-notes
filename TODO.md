...
| Q5 | Unit tests — all domain models and repositories | 🔧 | **60 unit tests written 2026-06-14** — `test/unit/`, all passing. Cover the logic behind this week's production bugs (`UserRole.fromString` least-privilege fallback, `AppUser` MFA gating, `UkTime` BST/GMT conversion + day windows, `ShiftType.forTime` London boundaries, `SyncService.isCloudSyncable` dev-mode guard) plus domain models (`Child` initials/age/JSON, `InspectorModule` scope-key **backend contract**, `ShiftCompletion` status, `InspectorGrant` + `ParentLink` isActive/displayName/fromJson). **Bug caught + fixed: `Child.ageYears` threw on a malformed DOB that still split into 3 parts (would crash a children-list row) → now `int.tryParse`.** Still to add: per-repository sync/upsert (needs in-memory Drift — sqlite3 native lib friction on Windows host; runs cleanly in CI/Linux). |
| Q6 | Widget tests — all screens | 🔧 | **First batch 2026-06-14** — `test/widget/` (10 tests, all passing): `OfflineBanner` online/offline rendering, `DailyNoteCard` content/tap/sync-indicator, and **`NavShell` role-based tabs for all 5 roles** (verifies each role sees only its permitted navigation — the security-relevant role isolation). Established the widget-test pattern (ProviderScope overrides, GoRouter wrapper, `initializeDateFormatting('en_GB')` for `DateFormat`, phone viewport for the bottom nav). Still to add: per-screen loading/error/empty/data states, form editors. |
| Q7 | Integration tests — offline→online sync, auth flow | ⬜ | 0 tests written. Guide specifies `test/integration/`. |
| Q8 | CI — `dart format` + `dart analyze` gate on every push | ✅ | Done 2026-06-14. `.github/workflows/ci.yml` runs on push/PR to main (Flutter 3.38.6, cached): `dart format --set-exit-if-changed lib test` → `flutter analyze` → `flutter test`. To make the gate green, normalised the codebase: `dart fix --apply` (29 const/null-aware fixes) + `dart format` (154 files). **`flutter analyze` now reports zero issues** (was 38 prefer_const infos). All 61 tests pass. |
...
|---|------|----------|-----------------|
| A1 | ~~`must_change_password` lives in user-writable `user_metadata`~~ ✅ **DONE (session 9).** Flag moved to admin-only `app_metadata` (set by manage-staff create/reset + bootstrap-home). New `change-password` Edge Function (JWT-verified) sets the password AND clears the flag atomically — the only way to clear it is to actually change the password, for your own account. AppUser reads app_metadata; SetPasswordScreen calls the function + refreshSession. | ✅ | Done — deploy `change-password` + redeploy manage-staff & bootstrap-home. NB: accounts created before the redeploy still carry the old user_metadata flag (now ignored) — re-provision or reset them if needed. |
| A2 | ~~No working password-reset loop~~ ✅ **DONE (session 9).** `manage-staff` `resetPassword` action (manager + AAL2 + home-scoped: new temp password + re-arm `must_change_password`) + a per-staff **Reset password** button on the Staff screen — closes the forgot-password loop for staff. *Parent/inspector reset still pending the accounts-list refactor (A5).* | ✅ | Done — redeploy `manage-staff` to activate. |
| A3 | ~~No vendor first-manager bootstrap~~ ✅ **DONE (session 9).** `bootstrap-home` Edge Function (secret-gated, deploy `--no-verify-jwt`): creates a `homes` row + first manager (must_change_password armed) for a brand-new home. Vendor-only, run once per home via curl — see `BOOTSTRAP_HOME.md`. The first manager then provisions everyone in-app. | ✅ | Done — deploy + set `BOOTSTRAP_SECRET` to use. |
...
| A5 | ~~Created-but-unlinked parents/inspectors aren't visible~~ ✅ **DONE (session 9).** Parent Access → Accounts \| Links tabs; Inspector Access → new Inspectors tab. Each account row (shared `ManagedAccountTile`) has a Reset password action (reuses manage-staff resetPassword), so A2 now covers parents + inspectors too. | ✅ | Done. |
