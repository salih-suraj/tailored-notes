...
void main() {
  ...
  group('AppUser.needsMfaChallenge', () {
    test('MFA roles need the challenge until the session is AAL2', () {
      expect(
        _user(UserRole.manager, mfaVerified: false).needsMfaChallenge,
        isTrue,
      );
      expect(
        _user(UserRole.inspector, mfaVerified: false).needsMfaChallenge,
        isTrue,
      );
    });
    ...
    test('MFA roles are cleared once the session is verified', () {
      expect(
        _user(UserRole.manager, mfaVerified: true).needsMfaChallenge,
        isFalse,
      );
      expect(
        _user(UserRole.inspector, mfaVerified: true).needsMfaChallenge,
        isFalse,
      );
    });
    ...
    test('non-MFA roles never need the challenge, verified or not', () {
      for (final role in [
        ...
        expect(
          _user(role, mfaVerified: false).needsMfaChallenge,
          isFalse,
